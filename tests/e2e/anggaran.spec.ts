...
async function deleteBudget(page: Page) {
  await page.getByRole("button", { name: "Ubah anggaran Hiburan" }).click();
  ...
  await expect(page.getByRole("heading", { name: "Belum ada anggaran di bulan ini" })).toBeVisible({ timeout: 15_000 });
}
...
test("buat anggaran wajib di bulan kosong, ubah nominalnya, lalu hapus", async ({ page }) => {
  ...
  // sisa percobaan sebelumnya yang gagal di tengah jalan dihapus lewat UI
  if (await row.isVisible()) await deleteBudget(page);
